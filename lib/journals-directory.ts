...
// Maps a note's free-text `author` display name to a stable username,
// without a Firestore round trip — covers the only 2 identities that
// can actually author a real note today (both founders, and
// @na-notesapp via the toggle in NoteForm.tsx). Used for the "new
// post" notification fan-out, which only needs a username (to query
// follows/subscriptions), not a uid — which is why this works for
// @na-notesapp even though it has no real Firebase Auth account.
// Returns null for an unmapped author name.
export function resolveAuthorUsername(displayName: string): string | null {
  if (displayName === NA_NOTESAPP_PROFILE.displayName) return NA_NOTESAPP_PROFILE.username;
  const founder = MANDATORY_JOURNALS.find(
    (j) => j.type === "person" && j.displayName === displayName
  );
  return founder ? founder.username : null;
}
