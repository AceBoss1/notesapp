    ...
    <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-14">
      <div className="flex items-center justify-between border-b-2 border-ink pb-4">
        ...
        <div className="flex gap-3">
          <Link
            ...
            className="bg-crimson text-paper font-ui font-semibold px-5 py-2.5 hover:bg-crimson-deep hover:text-paper transition-colors"
          >
            + New Journal Entry
          </Link>
          <button
      ...
      {!notes ? (
        ...
        <p className="mt-8 text-slate">
          No journal entries yet. Write the first one below — this list
          is the exact same content that shows up in Precheks' own
          Notes admin, since both apps share one Firestore database.
        </p>
      ) : (
          ...
          {notes.map((n) => (
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-[10px] font-mono uppercase tracking-wide px-2 py-0.5 ${
                      n.status === "published"
                        ? "bg-crimson/10 text-crimson-bright"
              ...
              <div className="flex gap-4 flex-shrink-0">
                <Link
                  href={`/admin/journals/${n.id}/edit`}
                  className="font-ui text-sm font-semibold text-crimson-bright hover:text-ink"
                >
                  Edit
                </Link>
                <button
